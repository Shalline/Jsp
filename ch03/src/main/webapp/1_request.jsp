...
<html>
	<head>
		...
		<%-- 
			날짜 : 2025/07/28
   			...
   			request 내장객체
   			- 클라이언트의 요청 정보를 갖는 JSP내장객체
   			- 클라이언트이 전송데이터를 수신처리하는 기능 제공
   			
   			GET 요청
   			- 기본 데이터 요청 방식, 전송데이터를  쿼리스트링으로 전송
   				(URL주소에 붙여 전송)
   			- 보안에 취약하지만 POST 전송대비 빠른 전송
   			
   			POST 요청
   			- 전송 데이터를 요청 객체의 body에 저장해서 전소오디는 방식
	...
	<body>
		...
		<h4>request 자주 사용하는 클라이언트 정보</h4>
			<p>
				해더정보 #1. <%= request.getHeader("User-Agent") %><br/>
				<!-- 혀재 머무르고 있는 페이지 직전의 요청 페이지(이전페이지) -->
				해더정보 #2. <%= request.getHeader("referer") %><br/>
				해더정보 #3. <%= request.getHeader("cookie") %><br/>
